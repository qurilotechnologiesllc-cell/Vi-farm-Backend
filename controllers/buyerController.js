...
/**
 * Helper function for review summery of vendor
 */
const getVendorReviewSummary = async (vendorId) => {
  // 1️⃣ vendor ke product ids
  const productIds = await Product.find({ vendor: vendorId })
    .select("_id")
    .lean()
  ...
  const rating = parseFloat((ratingAgg[0]?.avgRating || 0).toFixed(1));
...
const generateUpiPaymentUrl = (upiId, name, amount, transactionId) => {
  ...
  const transactionNote = encodeURIComponent(
    `Payment for Orders: ${transactionId}`,
  );
...
const getHomePageData = asyncHandler(async (req, res) => {
  ...
  const popularProducts = await Product.find({ status: "In Stock" })
    .sort({ rating: -1 })
    .limit(10)
    .populate("vendor", "name");
  const vendors = await User.find({ role: "Vendor", status: "Active" }).limit(
    5,
  );
...
const getProductDetails = asyncHandler(async (req, res) => {
  ...
  /* =========================
   3️⃣1️⃣ FETCH VENDOR ADDRESS (FROM VendorAddress)
========================= */
  const vendorAddressDoc = await VendorAddress.findOne({
    vendor: product.vendor._id,
  }).lean();

  const vendorCoords =
    vendorAddressDoc?.location?.coordinates ||
  ...
  ========================= */
  const vendorAddress = vendorAddressDoc
    ? {
      houseNumber: vendorAddressDoc.houseNumber || "",
      locality: vendorAddressDoc.locality || "",
      city: vendorAddressDoc.city || "",
      district: vendorAddressDoc.district || "",
      state: vendorAddressDoc.state || "",
      pinCode: vendorAddressDoc.pinCode || "",
      fullAddress: vendorAddressDoc.fullAddress || "",
      deliveryType: vendorAddressDoc.deliveryType || "Pickup",
      deliveryRadius: vendorAddressDoc.deliveryRadius || 0,
    }
    : null;
  ...
  return res.status(200).json({
    ...
    data: {
      ...
      vendor: {
        ...
        location: vendorAddressDoc?.location || null,
...
const getCategoriesWithProducts = asyncHandler(async (req, res) => {
  try {
    ...
    /* ==============================
       🔥 FETCH VENDOR ADDRESSES (SOURCE OF TRUTH)
    ============================== */
    ...
    const filteredResult = result.filter((cat) => cat.productCount > 0);
...
const getFreshAndPopularProducts = asyncHandler(async (req, res) => {
  ...
  const enriched = products.map((p) => {
    let distanceText = "N/A";
    let deliveryAllowed = false;
    let deliveryModes = ["Pickup"]; // default
...
const getFreshAndPopularVendors = asyncHandler(async (req, res) => {
  ...
  const vendorIds = [
    ...new Set(products.map(p => p.vendor?._id?.toString()).filter(Boolean))
  ];

  const vendorAddresses = await VendorAddress.find({
    vendor: { $in: vendorIds }
  })
    .select("vendor deliveryType")
    .lean();

  /* ==============================
   ⭐ REVIEWS (SAME AS getVendorsNearYou)
============================== */

  // 1️⃣ vendor → productIds
  const vendorProductMap = {};
  const allProductIds = [];

  for (const p of products) {
    if (!p.vendor?._id) continue;
    const vId = p.vendor._id.toString();
    if (!vendorProductMap[vId]) vendorProductMap[vId] = [];
    vendorProductMap[vId].push(p._id);
    allProductIds.push(p._id);
  }

  // 2️⃣ vendor → review data
  const vendorReviewMap = {};
  ...
  if (allProductIds.length) {
    ...
    for (const r of allReviews) {
      const vendorId = Object.keys(vendorProductMap).find((vId) =>
        vendorProductMap[vId].some((pId) => pId.equals(r.product))
      );

      if (!vendorId) continue;

      if (!vendorReviewMap[vendorId]) {
        vendorReviewMap[vendorId] = {
          totalRating: 0,
          count: 0,
          reviews: [],
        };
      ...
      vendorReviewMap[vendorId].totalRating += r.rating;
      vendorReviewMap[vendorId].count += 1;

      // latest 3 reviews only
      if (vendorReviewMap[vendorId].reviews.length < 3) {
        vendorReviewMap[vendorId].reviews.push({
          _id: r._id,
          user: r.user,
          rating: r.rating,
          comment: r.comment || "",
  ...
  const vendorDeliveryMap = {};
  vendorAddresses.forEach(va => {
    vendorDeliveryMap[va.vendor.toString()] = va.deliveryType;
  });
  ...
  for (const p of products) {
    ...
    if (!mapVendor[vId]) {
      const deliveryType = vendorDeliveryMap[vId] || "Pickup";
      mapVendor[vId] = {
        id: vId,
        name: p.vendor.name,
        profilePicture:
      ...
      // ⭐ STEP 3 – yahin review data attach karo
      const reviewData = vendorReviewMap[vId];

      if (reviewData) {
        mapVendor[vId].rating = reviewData.count
          ? Number((reviewData.totalRating / reviewData.count).toFixed(1))
          : 0;

        mapVendor[vId].reviewCount = reviewData.count;
        mapVendor[vId].reviews = reviewData.reviews;
      }
...
//     ============================== */
//     const buyerUser = await User.findById(buyerId).select("location").lean();
...
//       if (Array.isArray(coords) && coords.length === 2 && !coords.includes(0)) {
  ...
  try {
    ...
    const formattedProducts = products
      ...
      .map((p) => {
        const vendorId = p.vendor?._id?.toString();
        const vendorData = vendorMap[vendorId];
        const vendorAddr = vendorAddressMap[vendorId];
...
const getAllAroundIndiaProducts = asyncHandler(async (req, res) => {
  ...
  if (
    ...
    return res.status(400).json({
      success: false,
      message:
        "Buyer location not found. Please update your profile location first.",
    });
  }
...
const getAllAroundIndiaProducts = asyncHandler(async (req, res) => {
  ...
  // 🧮 Step 4: Add distance + pickup/delivery (ADD-ON ONLY)
  const enrichedProducts = products.map((p) => {
    let distanceText = "N/A";
    let distanceKm = null;
    ...
    // 🚚 DELIVERY DECISION — ONLY VendorAddress
    let deliveryAllowed = false;
    let deliveryModes = ["Pickup"];
    ...
    return {
      ...
      // ⭐ ONLY ADDITION (as asked)
      deliveryAllowed,
      deliveryModes,
    };
  });

  // 🧠 Step 5: Sort by nearest (NO CHANGE)
  const sortedProducts = enrichedProducts.sort((a, b) => {
    const da = parseFloat(a.distance);
    const db = parseFloat(b.distance);
    if (isNaN(da)) return 1;
...
const getVendorsNearYou = asyncHandler(async (req, res) => {
  try {
    /* ==============================
   🔒 GLOBAL PRODUCT FILTER (NEARBY)
    ============================== */
    const validVendorIds = await Product.distinct("vendor", {
      status: "In Stock",
    });
    ...
    for (const addr of vendorAddresses) {
      vendorDeliveryMap[addr.vendor.toString()] = addr.deliveryType || "Pickup";
    }

    /* ==============================
       4️⃣2️⃣ REVIEWS (BULK – NO EXTRA FLOW CHANGE)
    ============================== */
    const products = await Product.find({
      vendor: { $in: vendorIds },
    })
    ...
    const enrichedVendors = await Promise.all(
      vendors.map(async (v) => {
        ...
        const deliveryType = vendorDeliveryMap[v._id.toString()] || "Pickup";
        ...
        const reviewData = vendorReviewMap[v._id.toString()] || {
          totalRating: 0,
          count: 0,
          reviews: [],
        };

        const avgRating = reviewData.count
          ? parseFloat(
        ...
        return {
          ...
          categories:
            categoryNames.length > 0 ? categoryNames : "No categories listed",

          deliveryRegion: v.vendorDetails?.deliveryRegion || 0,
          // 🔥 NEW (SAME AS OTHER API)
          deliveryOptions,
          // ⭐⭐⭐ REVIEWS ADDED
          rating: avgRating,
...
============================== */
    ...
    if (q) {
      ...
      // 1️⃣ Vendor name match
      const vendorsByName = await User.find({
        role: "Vendor",
        status: "Active",
      ...
      const vendorIdsByName = vendorsByName.map((v) => v._id);

      // 2️⃣ Category name match
      ...
      // 3️⃣ Product name OR category match
      let vendorIdsByProduct = [];
      if (categoryIds.length || searchText) {
        vendorIdsByProduct = await Product.distinct("vendor", {
          status: "In Stock",
          $or: [
            { name: { $regex: searchText, $options: "i" } },
      ...
      // 🔥 UNION of all vendor IDs
      vendorIdsFromSearch = [
        ...new Set([
          ...vendorIdsByName.map(String),
          ...vendorIdsByProduct.map(String),
        ]),
      ];
    }
    ...
    if (vendorIdsFromSearch.length) {
      query._id = { $in: vendorIdsFromSearch };
    }

    /* ==============================
       3️⃣ CATEGORY FILTER (FIXED)
    ============================== */
    if (category) {
      let categoryId;
      ...
      const vendorIdsByCategory = await Product.distinct("vendor", {
        category: categoryId,
        status: "In Stock",
      });

      if (!vendorIdsByCategory.length) {
        return res.json({
          success: true,
          count: 0,
          vendors: [],
      ...
      query._id = { $in: vendorIdsByCategory };
    }

    /* ==============================
       4️⃣ FETCH VENDORS (MISSING STEP)
    ============================== */
    const vendors = await User.find(query)
      .select("name profilePicture location vendorDetails address")
      .lean();
    ...
    /* ==============================
   5️⃣1️⃣ FETCH REVIEWS (BULK, NO FLOW CHANGE)
============================== */
    ...
    const enrichedVendors = vendors.map((vendor) => {
      ...
      const distanceText = getDistanceText(buyerCoords, vendorCoords);
      const deliveryType = vendorDeliveryMap[vendor._id.toString()] || "Pickup";
      ...
      // 🔥 ADD REVIEWS
      const reviewData = vendorReviewMap[vendor._id.toString()] || {
        totalRating: 0,
        count: 0,
        reviews: [],
      };
...
const getCartItems = asyncHandler(async (req, res) => {
  try {
    ...
    // ---------------- BUYER INFO ----------------
    const buyer = await User.findById(userId).select("address location").lean();
    ...
    if (cart.couponCode) {
      ...
      if (coupon) {
        ...
        validItems.forEach((item) => {
          ...
          if (isAll || isCategoryNameMatch || isProductMatch) {
            eligibleMRP += item.mrp * item.quantity;
          }
        });

        if (eligibleMRP > 0) {
          if (coupon.discount.type === "Percentage") {
            couponDiscount = (eligibleMRP * coupon.discount.value) / 100;
          } else {
            couponDiscount = Math.min(coupon.discount.value, eligibleMRP);
          }
        }
      }
    }
...
const placePickupOrder = asyncHandler(async (req, res) => {
  try {
    ...
    // AUTO APPLY COUPON
    // APPLY COUPON ONLY IF CLIENT PROVIDES IT
    ...
    if (appliedCouponCode) {
      ...
      if (
        ...
        used &&
        used.count >= coupon.usageLimitPerUser
      ) {
        return res.status(400).json({
          success: false,
...
const placeOrder = asyncHandler(async (req, res) => {
  try {
    const userId = req.user._id;
    const { comments, paymentMethod, couponCode } = req.body;
...
=============================== */
    ...
    /* ===============================
       8️⃣ COUPON
    =============================== */
    const appliedCouponCode = pref?.couponCode?.trim()?.toUpperCase() || null;
    const rawCouponCode = couponCode?.trim()?.toUpperCase() || null;
    ...
    if (rawCouponCode) {
      ...
      if (
        coupon.vendor &&
        coupon.vendor.toString() !== selectedVendorId.toString()
      ) {
        return res.status(400).json({
          success: false,
          message: "This coupon is not applicable for selected vendor.",
        });
      }
      ...
      if (
        ...
        userUsage &&
        userUsage.count >= coupon.usageLimitPerUser
      ) {
        return res.status(400).json({
          success: false,
          message: `You have already used this coupon ${coupon.usageLimitPerUser} times.`,
        });
      }
    }
...
const reviewOrder = asyncHandler(async (req, res) => {
  ...
  const cart = await Cart.findOne({ user: userId })
    .populate({
      path: "items.product",
      select:
        "name price images vendor weightPerPiece category variety sku unit",
      populate: {
  ...
  /* ===============================
     5️⃣ FINAL COUPON
  =============================== */
  const finalCouponCode = prefCoupon ? prefCoupon.toUpperCase() : null;
  ...
  const finalItems = updatedItems.map((i) => {
    ...
    const est = calculateEstimatedDelivery(vendor, {
...
const applyCouponToCart = asyncHandler(async (req, res) => {
  ...
  if (new Date(coupon.startDate) > new Date())
    return res
      .status(400)
      .json({ success: false, message: "Coupon not started yet." });
  ...
  // Save coupon in cart
  await Cart.updateOne({ user: userId }, { $set: { couponCode: couponCode } });
...
const addItemToCart = asyncHandler(async (req, res) => {
  ...
  const product = await Product.findById(productId).select(
    "name price weightPerPiece vendor status images unit variety",
  );
  ...
  if (existingItemIndex > -1) {
    ...
    cart.items[existingItemIndex].quantity = increaseToNextInteger(currentQty);
...
const getVendorProfileForBuyer = asyncHandler(async (req, res) => {
  ...
  const listedProductsRaw = await Product.find(productFilter)
    .select(
      "name category variety price quantity unit images rating weightPerPiece",
    )
    .populate("category", "name image")
    .sort({ rating: -1 })
    .limit(20)
  ...
  const vendorAddress = await VendorAddress.findOne({ vendor: vendorId }).lean();
...
const getProductReviews = asyncHandler(async (req, res) => {
  ...
  const product = await Product.findById(productId)
    .select(
      "name variety category images weightPerPiece rating price description",
    )
    .lean();
  ...
  const averageRating =
    totalReviews > 0
      ? (
        reviewsRaw.reduce((sum, review) => sum + review.rating, 0) /
        totalReviews
      ).toFixed(1)
      : 0;
  ...
  res.status(200).json({
    ...
    data: {
      ...
      product: {
        ...
        description:
          product.description || "Product description not available.", // Product description
...
const getOrderDetails = asyncHandler(async (req, res) => {
  ...
  const products = await Product.find({ _id: { $in: productIds } })
    .select(
      "name images variety price unit vendor quantity weightPerPiece description category",
    )
    .populate(
      "vendor",
      "name profilePicture mobileNumber address vendorDetails.about location state deliveryRegion",
    )
    .lean();
...
const getAvailableCouponsForBuyer = asyncHandler(async (req, res) => {
  try {
    ...
    const coupons = await Coupon.find({
      ...
      .select(
        "code discount minimumOrder usageLimitPerUser totalUsageLimit startDate expiryDate appliesTo",
      )
...
const getHighlightedCoupon = asyncHandler(async (req, res) => {
  try {
    ...
    // ✅ SIRF admin + selected vendor coupons
    const coupons = await Coupon.find({
      status: "Active",
      startDate: { $lte: now },
      expiryDate: { $gte: now },
      $or: [
        { vendor: null }, // ADMIN coupon
        { vendor: new mongoose.Types.ObjectId(vendorId) }, // SELECTED vendor
      ],
    }).lean();
...
const getHighlightedCouponByVendor = asyncHandler(async (req, res) => {
  ...
  const coupons = await getVisibleCouponsForVendors([
...
const addToWishlist = asyncHandler(async (req, res) => {
  ...
  const product = await Product.findById(productId).populate(
    "vendor",
    "name profilePicture mobileNumber vendorDetails location address rating",
  );
  if (!product) {
    return res
      .status(404)
...
const startCheckout = asyncHandler(async (req, res) => {
  ...
  if (validItems.length === 0) {
    return res.status(400).json({
      success: false,
      message: "All items in cart are invalid or deleted.",
    });
  }
  ...
  if (ratingChanged && updatedReview.product && updatedReview.product._id) {
    try {
      const agg = await Review.aggregate([
        {
          $match: {
            product: mongoose.Types.ObjectId(updatedReview.product._id),
          },
        },
        {
          $group: {
...
const deleteReview = asyncHandler(async (req, res) => {
  ...
  if (onlineUsers && onlineUsers[req.user._id]) {
    io.to(onlineUsers[req.user._id].socketId).emit(
      "notification",
      notification,
    );
  }
  ...
  try {
    ...
    if (latitude !== undefined && longitude !== undefined) {
      ...
      if (coords && coords.length === 2) {
        ...
        console.warn(
          "⚠️ Geocoding failed → saving address without coordinates",
        );
      }
    }
...
const selectPickupSlot = asyncHandler(async (req, res) => {
  ...
  if (!vendorId || !date || !startTime || !endTime) {
    return res
      .status(400)
      .json({ success: false, message: "All slot fields are required." });
  }
...
const getPickupLocationDetails = asyncHandler(async (req, res) => {
  ...
  const buyerAddress =
    defaultBuyerAddress || (await Address.findOne({ user: userId }).lean());
...
const getPickupLocationDetailsPost = asyncHandler(async (req, res) => {
  ...
  if (
    ...
    const dist = calculateDistanceKm(lat, lng, vendorLat, vendorLng);
...
const getProductById = asyncHandler(async (req, res) => {
  ...
  const product = await Product.findById(id)
    .populate({
      path: "vendor",
      select:
        "name mobileNumber email address vendorDetails.about profilePicture",
    })
    .lean();
...
const donateToAdmin = asyncHandler(async (req, res) => {
  ...
  if (onlineUsers[admin._id]) {
    io.to(onlineUsers[admin._id].socketId).emit(
      "notification",
      adminNotification,
    );
  }
...
const getDonationsReceived = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    const donations = await Donation.find()
      .select(
        "donor admin amount message paymentMethod transactionRef status createdAt",
      )
    ...
    res
      .status(500)
      .json({ success: false, message: "Failed to retrieve donation data." });
  }
});
...
const searchProductsByName = asyncHandler(async (req, res) => {
  ...
  const results = allProducts
    .filter((p) =>
      [p.name, p.variety, p.description, p.vendor?.name, p.category?.name].some(
...
const searchProductsByName = asyncHandler(async (req, res) => {
  ...
  // 🔹 Fetch vendor addresses ONCE
  const vendorIds = allProducts.map((p) => p.vendor?._id).filter(Boolean);
  ...
  // 🔍 Filter + add distance + delivery info
  const results = allProducts
    .filter((p) =>
      [p.name, p.variety, p.description, p.vendor?.name, p.category?.name].some(
        (v) => v?.toLowerCase().includes(q),
    ...
    .map((p) => {
      ...
      if (
        buyerCoords?.length === 2 &&
        vendorAddr?.location?.coordinates?.length === 2
      ) {
        const [buyerLng, buyerLat] = buyerCoords;
        const [vendorLng, vendorLat] = vendorAddr.location.coordinates;
      ...
      return {
        ...p,
        distance, // 👈 existing
        deliveryAllowed, // ⭐ NEW (safe)
        deliveryModes, // ⭐ NEW (safe)
      };
    });
...
const markOrderPaid = asyncHandler(async (req, res) => {
  ...
  res.status(200).json({
    success: true,
    message:
      "Payment confirmed successfully. Notifications sent to buyer and vendor.",
  });
});

//List vendor for buyer in map
...
module.exports = {
  ...
  getBuyerProfile,
  placePickupOrder,
  getDeliveryChargeController,
  updateBuyerProfile,
  selectVendorInCart,
  logout,
  getFreshAndPopularVendors,
  getOrderDetails,
  ...
  getVendorProfileForBuyer,
  getProductReviews,
  getAvailableCouponsForBuyer,
  getBuyerOrders,
  getLocalBestProducts,
  getAllAroundIndiaProducts,
  getSmartPicks,
  getCouponsByProductId,
  getOrderDetails,
  searchProducts,
  getFreshAndPopularProducts,
