    ...
    const todayRevenueResult = await Order.aggregate([
      {
        $match: {
          ...
          createdAt: { $gte: startOfToday, $lte: endOfToday },
          orderStatus: {
            $in: ["Delivered", "Confirmed", "Completed"],
          },
        },
      },
      { $group: { _id: null, total: { $sum: "$totalPrice" } } },
...
const getVendorDashboardAnalytics = asyncHandler(async (req, res) => {
  ...
  const endOfLastYearMonth = new Date(
    currentYear - 1,
    currentMonth + 1,
    0,
    23,
  ...
  const currentMonthCustomers = await getUniqueCustomerCount(
    startOfCurrentMonth,
    endOfCurrentMonth,
  );
  const lastYearMonthCustomers = await getUniqueCustomerCount(
  ...
  if (lastYearMonthCustomers > 0) {
    percentageChange =
      ((currentMonthCustomers - lastYearMonthCustomers) /
        lastYearMonthCustomers) *
      100;
...
const getVendorOrderStats = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    const monthlyOrders = Array.from({ length: 12 }, (_, i) => {
      ...
      return monthData
        ? { orders: monthData.count, revenue: monthData.revenue }
        : { orders: 0, revenue: 0 };
    });
...
const getMonthlyOrders = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    res
      ...
      .json({
        success: false,
        message: "Failed to fetch monthly order data.",
        error: error.message,
      });
  }
});
...
const getVendorOrders = asyncHandler(async (req, res) => {
  const orders = await Order.find({ vendor: req.user._id })
    ...
    .populate(
      "products.product",
      "name price mobileNumber quantity unit variety",
    ); // populate product details inside products array
...
const createCoupon = asyncHandler(async (req, res) => {
  ...
  if (!["Fixed", "Percentage"].includes(discountType)) {
    return res
      .status(400)
      .json({ success: false, message: "Invalid discountType." });
  }
  ...
  if (Array.isArray(appliesTo) && appliesTo.length > 0) {
    const categories = await Category.find({
      $or: [
        {
          _id: {
            $in: appliesTo.filter((id) => mongoose.Types.ObjectId.isValid(id)),
          },
        },
        { name: { $in: appliesTo } },
      ],
  ...
  const formatted = {
    ...
    // products
    products:
      newCoupon.applicableProducts?.map((p) => ({
        id: p._id,
        name: p.name,
        price: p.price,
...
const updateVendorCoupon = asyncHandler(async (req, res) => {
  ...
  if (Array.isArray(appliesTo) && appliesTo.length > 0) {
    ...
    if (categories.length === 0) {
      return res
        .status(400)
        .json({ success: false, message: "Invalid category names." });
    }
  ...
  if (appliesTo !== undefined) {
    ...
    // CATEGORY BASED
      ...
      if (productIds.length === 0) {
        ...
        if (products.length !== productIds.length) {
          return res
            .status(403)
            .json({ success: false, message: "Invalid product selection." });
        }
    ...
    // SPECIFIC PRODUCT
      ...
      return res
        .status(400)
        .json({ success: false, message: "Invalid appliesTo." });
    }
  }
...
const getUserProfile = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    if (user.role === "Vendor") {
      ...
      const vendorProducts = await Product.find({ vendor: user._id }).select(
        "_id",
      );
  ...
  try {
    ...
    if (req.body.address) {
      try {
        ...
        return res
          .status(400)
          .json({ success: false, message: "Invalid address JSON format" });
      }
    }
...
const updateLocationDetails = asyncHandler(async (req, res) => {
  try {
    ...
    if (enableDelivery === true) {
      radius = deliveryRadius ? Number(deliveryRadius) : 0;

      if (isNaN(radius) || radius < 0) {
        return res.status(400).json({
          success: false,
          message: "Delivery radius must be a valid number",
        });
      }

      deliveryType = radius > 0 ? "Both" : "Delivery";
    }

    /* ===============================
       LOAD DOCUMENT (IMPORTANT)
    =============================== */
    let address = await VendorAddress.findOne({
      vendor: req.user._id,
    });
    ...
    /* ===============================
       SAVE (HOOK RUNS HERE 🔥)
    =============================== */
    await address.save(); // <-- THIS IS THE KEY

    /* ===============================
       SYNC TO USER (OPTIONAL)
    =============================== */
    if (address.location?.coordinates?.[0] !== 0) {
      await User.findByIdAndUpdate(req.user._id, {
        $set: {
          location: address.location,
          "vendorDetails.deliveryRegion": radius,
          "vendorDetails.location": address.fullAddress,
        },
      });
    }
...
const reportUserIssue = asyncHandler(async (req, res) => {
  ...
  res
    .status(201)
    .json({ success: true, message: "Issue reported successfully." });
});
